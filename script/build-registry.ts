...
const getComponentFiles = async (files: File[], registryType: string) => {
    const filesArrayPromises = (files ?? []).map(async (file) => {
        if (typeof file === "string") {
            ...
            const targetPath = file.target || `/components/FingUIComponents/${fileName}`;
